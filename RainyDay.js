...
function RainyDay(a) {
    ...
    var b = "string" == typeof a.image ? document.getElementById(a.image) : a.image;
...
RainyDay.prototype.destroy = function() {
    ...
    this.bckStyle && (this.imgSource.style.background = this.bckStyle.background),
...
RainyDay.prototype.initialize = function(a) {
    var b = this.imgSource || a.parentElement || document.getElementsByTagName("body")[0]
...
RainyDay.prototype.prepareCanvas = function() {
    ...
    this.img.style.zIndex ? (a.style.zIndex = this.img.style.zIndex,
    this.img.style.zIndex += 1) : a.style.zIndex = 99,
    this.imgSource ? (this.options.parentElement.parentNode.insertBefore(a, this.imgSource),
    ...
    this.imgSource.style.width = this.options.width + "px") : this.options.parentElement.appendChild(a),
...
RainyDay.prototype.checkSize = function() {
    ...
    this.options.parentElement.style.zIndex && (this.canvas.style.zIndex = this.options.parentElement.style.zIndex),
    h === d && i === e || (this.canvas.width = d,
    this.canvas.height = e,
    this.glass.width = this.canvas.width,
    this.glass.height = this.canvas.height,
    this.prepareBackground(),
    this.prepareReflections()),
    j === f && k === g || (this.canvas.style.left = f,
    this.canvas.style.top = g)
}
,
...
RainyDay.prototype.prepareReflections = function() {
    ...
    a.drawImage(this.imgDownscaled, 0, 0, this.imgDownscaled.width, this.imgDownscaled.height, 0, 0, this.reflected.width, this.reflected.height)
}
,
...
RainyDay.prototype.rain = function(a, b) {
    if (this.reflection !== this.REFLECTION_NONE && this.prepareReflections(),
    this.animateDrops(),
    this.presets = a,
    this.PRIVATE_GRAVITY_FORCE_FACTOR_Y = .001 * this.options.fps / 25,
    this.PRIVATE_GRAVITY_FORCE_FACTOR_X = (Math.PI / 2 - this.options.gravityAngle) * (.001 * this.options.fps) / 50,
    this.options.enableCollisions) {
        ...
        if (c > 0) {
            ...
            this.options.enableCollisions = !1
    }
...
RainyDay.prototype.putDrop = function(a) {
    a.draw(),
    this.gravity && a.r > this.options.gravityThreshold && (this.options.enableCollisions && this.matrix.update(a),
    this.drops.push(a))
}
,
...
Drop.prototype.draw = function() {
    ...
    var a = this.r;
    if (this.r = Math.floor(.95 * this.r),
    this.r < 3)
        ...
        this.context.arc(this.x, this.y, .9 * this.r, 0, 2 * Math.PI, !0),
        this.context.closePath();
    this.context.clip(),
    this.r = a,
...
Drop.prototype.clear = function(a) {
    ...
    a ? (this.terminate = !0,
    !0) : this.y - this.r > this.rainyday.canvas.height || this.x - this.r > this.rainyday.canvas.width || this.x + this.r < 0
}
,
Drop.prototype.animate = function() {
    ...
    if (!a && this.rainyday.trail && this.rainyday.trail(this),
...
RainyDay.prototype.TRAIL_DROPS = function(a) {
    (!a.trailY || a.y - a.trailY >= 100 * Math.random() * a.r) && (a.trailY = a.y,
    this.putDrop(new Drop(this,Math.floor(a.x + (2 * Math.random() - 1) * Math.random()),a.y - a.r - 5,Math.ceil(a.r / 5),0)))
}
,
RainyDay.prototype.TRAIL_SMUDGE = function(a) {
    var b = a.y - a.r - 3
      , c = a.x - Math.floor(a.r / 2) + 2 * Math.random();
    b < 0 || c < 0 || this.context.drawImage(this.clearbackground, c, b, a.r, 2, c, b, a.r, 2)
}
,
...
RainyDay.prototype.GRAVITY_LINEAR = function(a) {
    return !!this.clearDrop(a) || (a.yspeed ? (a.yspeed += this.PRIVATE_GRAVITY_FORCE_FACTOR_Y * Math.floor(a.r),
    a.xspeed += Math.floor(this.PRIVATE_GRAVITY_FORCE_FACTOR_X * Math.floor(a.r))) : (a.yspeed = this.PRIVATE_GRAVITY_FORCE_FACTOR_Y,
...
RainyDay.prototype.GRAVITY_NON_LINEAR = function(a) {
    return !!this.clearDrop(a) || (a.collided ? (a.collided = !1,
    a.seed = Math.floor(a.r * Math.random() * this.options.fps),
    a.skipping = !1,
    a.slowing = !1) : (!a.seed || a.seed < 0) && (a.seed = Math.floor(a.r * Math.random() * this.options.fps),
    a.skipping = a.skipping === !1,
    a.slowing = !0),
    a.seed--,
    a.yspeed ? a.slowing ? (a.yspeed /= 1.1,
    a.xspeed /= 1.1,
    a.yspeed < this.PRIVATE_GRAVITY_FORCE_FACTOR_Y && (a.slowing = !1)) : a.skipping ? (a.yspeed = this.PRIVATE_GRAVITY_FORCE_FACTOR_Y,
    ...
    0 !== this.options.gravityAngleVariance && (a.xspeed += (2 * Math.random() - 1) * a.yspeed * this.options.gravityAngleVariance),
    a.y += Math.floor(a.yspeed),
...
RainyDay.prototype.REFLECTION_MINIATURE = function(a) {
    ...
    this.context.drawImage(this.reflected, Math.floor(b), Math.floor(c), Math.floor(d), Math.floor(e), Math.floor(f), Math.floor(g), 2 * a.r, 2 * a.r)
}
,
RainyDay.prototype.COLLISION_SIMPLE = function(a, b) {
    for (var c, d = b; null != d; ) {
        var e = d.drop
          ...
          , g = a.x - e.x
          , h = a.y - e.y;
        if (Math.abs(g) < f && Math.abs(h) < f && Math.sqrt(Math.pow(a.x - e.x, 2) + Math.pow(a.y - e.y, 2)) < a.r + e.r) {
            c = e;
            break
        }
        d = d.next
...
CollisionMatrix.prototype.update = function(a, b) {
    if (a.gid) {
        if (!this.matrix[a.gmx] || !this.matrix[a.gmx][a.gmy])
            return null;
        if (this.matrix[a.gmx][a.gmy].remove(a),
        b)
            return null;
        if (a.gmx = Math.floor(a.x / this.resolution),
        ...
        if (a.gid = Math.random().toString(36).substr(2, 9),
